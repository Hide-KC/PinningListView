 ...
 */
open class PinningListDecorationForDataBinding<E, T, H>(private val listener: PinningListDBListener<E, T, H>) :
  RecyclerView.ItemDecoration() {

  private var header = Pair<Int?, ViewDataBinding?>(null, null)

  fun getHeaderWidth(): Int? = header.second?.root?.measuredWidth

  fun getHeaderHeight(): Int? = header.second?.root?.measuredHeight
  ...
  override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
    ...
    val currentHeader = when (val layoutResId = listener.headerLayout ?: return) {
      header.first -> {
        ...
        header = Pair(layoutResId, binding)
        binding
      }
    } ?: return
    ...
    if (nextHeaderTop != null
      ...
      moveHeader(c, currentHeader.root, nextHeaderTop)
    } else {
